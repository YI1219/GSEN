...
def rl_moe_average_weights(w, rl_w_actions):
    # rl_w_actions: local model number x 2 | [[moe_index, average_percentage], ...]
    w_avg = {}
    
    for i in range(len(w)):
        moe_index = rl_w_actions[i][0]
        if str(moe_index) not in w_avg:
            w_avg[str(moe_index)] = copy.deepcopy(w[i])
            for key in w[i].keys():
                w_avg[str(moe_index)][key] = w_avg[str(moe_index)][key] * rl_w_actions[i][1]
        else:
            for key in w[i].keys():
                w_avg[str(moe_index)][key] += w[i][key] * rl_w_actions[i][1]
    return w_avg
