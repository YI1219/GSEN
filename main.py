...
from models.blip24fed import Blip24Fed
...
def args_parser():
    ...
    parser.add_argument("--temperature", type=float, default=0.26,
                        help="the temperature parameter for contrastive loss")
    parser.add_argument("--ts", type=float, default=0.81,
                        help="the temperature parameter for js loss in teacher model")
    parser.add_argument("--ss", type=float, default=0.81,
    ...
    parser.add_argument('--train_ep_private', type=int,
                        default=10, help="the number of local episodes: E")
    parser.add_argument('--learning_rate_img_private', type=float,
                        default=1e-5, metavar='N', help='learning_rate')  # 0.01
    parser.add_argument('--learning_rate_txt_private', type=float,
                        default=1e-5, metavar='N', help='learning_rate')
    parser.add_argument('--weight_decay_p', default=1e-6,
    ...
    parser.add_argument('--alg', type=str,
                        default='FedUCCH', help="algorithms")
    parser.add_argument('--noniid', type=str, default='--', help="noniid")
    parser.add_argument('--batch_size', type=int,
                        default=256, help="local batch size")
    parser.add_argument('--learning_rate_img', type=float,
                        default=1e-4, metavar='N', help='learning_rate')  # 0.01
    parser.add_argument('--learning_rate_txt', type=float,
                        default=1e-4, metavar='N', help='learning_rate')
    parser.add_argument('--momentum', type=float, default=0.4,
                        help='SGD momentum (default: 0.9)')
    parser.add_argument('--weight_decay', default=1e-6,
                        type=float, help='weight_decay')
    parser.add_argument('--device', default="cuda",
                        type=str, help="cpu, cuda, or others")
    parser.add_argument('--optimizer', type=str,
                        default='Adam', help="type of optimizer")
    parser.add_argument('--save_model_path',
                        default='./checkpoint/', help='path to save_model')
    parser.add_argument('--nce', type=int, default=0, help="nce")
    parser.add_argument('--dataset', type=str, default='MIR-FLICKR25K',
                        help="name of dataset, e.g. MIR-FLICKR25K")
    parser.add_argument('--data_dir', type=str, default='./data/',
                        help="name of dataset, default: './data/'")
    parser.add_argument('--num_classes', type=int,
                        default=10, help="number of classes")
    parser.add_argument('--feature_iid', type=int, default=0,
                        help='Default set to feature non-IID. Set to 1 for feature IID.')
    parser.add_argument('--label_iid', type=int, default=0,
                        help='Default set to label non-IID. Set to 1 for label IID.')
    ...
    parser.add_argument(
        '--num_hiden_layers', default=[3, 2], nargs='+', help='<Required> Number of hiden lyaers')
...
class LocalUpdateDJSRH(object):
    def __init__(self, args, id, dataset, idxs):
        ...
        self.contrast = NCEAverage(
            args.bit, self.m_size, args.K, args.T, args.momentum)
    ...
    def update_weights_UCCH_new(self, args, idx, global_protos, model_img, model_txt, private_img_model, private_txt_model):
        ...
        if args.optimizer == "sgd":
            optimizer_i_s = torch.optim.SGD(filter(
                lambda p: p.requires_grad, model_img.parameters()), lr=args.learning_rate_img)
            optimizer_t_s = torch.optim.SGD(filter(
                lambda p: p.requires_grad, model_txt.parameters()), lr=args.learning_rate_txt)
            optimizer_i_private = torch.optim.SGD(filter(
                lambda p: p.requires_grad, private_img_model.parameters()), lr=args.learning_rate_img_private)
            optimizer_t_private = torch.optim.SGD(filter(
                lambda p: p.requires_grad, private_txt_model.parameters()), lr=args.learning_rate_txt_private)
        else:
            optimizer_i = torch.optim.Adam(global_img_model.parameters(
            ), lr=args.learning_rate_img, weight_decay=args.weight_decay)
            optimizer_t = torch.optim.Adam(global_txt_model.parameters(
            ), lr=args.learning_rate_txt, weight_decay=args.weight_decay)
            optimizer_i_p = torch.optim.Adam(private_img_model.parameters(
            ), lr=args.learning_rate_img, weight_decay=args.weight_decay)
            optimizer_t_p = torch.optim.Adam(private_txt_model.parameters(
            ), lr=args.learning_rate_txt, weight_decay=args.weight_decay)
        ...
        for epoch in range(args.train_ep_private):
            for batch_idx, (img_F, txt_F, labels, index) in enumerate(self.dataloader_train):
                ...
                out_l, out_ab = self.contrast(
                    images_outputs, texts_outputs, idx, epoch=epoch-args.warmup_epoch)
                ...
                loss_cluster = self.cluster_contrast(hid_I_p, centroid.to(device), peso_labels[-batch_size:].to(device), batch_size) +\
                    self.cluster_contrast(hid_T_p, centroid.to(
                        device), peso_labels[-batch_size:].to(device), batch_size)
                ...
                loss_kd_s = self.loss_mse(img_stu_norm.mm(img_stu_norm.t()), img_tea_norm.mm(img_tea_norm.t())) + \
                    ...
                    self.loss_mse(txt_stu_norm.mm(img_stu_norm.t()),
                                  txt_tea_norm.mm(img_tea_norm.t()))
        ...
        for epoch in range(args.train_ep):
            ...
            for batch_idx, (img_F, txt_F, labels, index) in enumerate(self.dataloader_train):
                ...
                out_l, out_ab = self.contrast(
                    images_outputs_g, texts_outputs_g, idx, epoch=epoch-args.warmup_epoch)
                l_loss = self.criterion_contrast(out_l)
                ab_loss = self.criterion_contrast(out_ab)
                Lc = l_loss + ab_loss
                ...
                if len(global_protos) != 0:
                    ...
                    queue_v, out = update_queue(self.queue_v_local, fushed)
                    # , verbose=1)
                    kmeans = KMeans(n_clusters=24, mode='cosine')
                    cluster_r = kmeans.fit_predict(queue_v)
                    local_protos = kmeans.centroids

                    loss_RG = self.js_loss(
                        hid_I, hid_T, global_protos[0], t=self.args.temperature, t2=self.args.ts)
                    loss_RL = self.js_loss(
                        hid_I, hid_T, local_protos, t=self.args.temperature, t2=self.args.ss)
    ...
    def js_loss(self, x1, x2, xa, t=0.1, t2=0.01):
        ...
        pred_sim1 = torch.mm(F.normalize(x1, dim=1),
                             F.normalize(xa, dim=1).t())
        inputs1 = F.log_softmax(pred_sim1 / t, dim=1)
        pred_sim2 = torch.mm(F.normalize(x2, dim=1),
                             F.normalize(xa, dim=1).t())
        inputs2 = F.log_softmax(pred_sim2 / t, dim=1)
        target_js = (F.softmax(pred_sim1 / t2, dim=1) +
                     F.softmax(pred_sim2 / t2, dim=1)) / 2
...
class LocalTest(object):
    def __init__(self, args, id, dataloaders, idcs_retrieval, idcs_query):
        ...
        self.testloader, self.databaseloader = self.test_split(
            args, dataloaders, idcs_retrieval[id], idcs_query[id])
        self.device = args.device
    ...
    def test_split(self, args, dataloader, idcs_retrieval, idcs_query):
        ...
        imgs = {'query': query_x_for_this_client,
                'database': retrieval_x_for_this_client}
        texts = {'query': query_y_for_this_client,
                 'database': retrieval_y_for_this_client}
        labels = {'query': query_L_for_this_client,
                  'database': retrieval_L_for_this_client}
        ...
        dataloader = {x: DataLoader(dataset[x], batch_size=args.batch_size,
                                    shuffle=shuffle[x], num_workers=3) for x in ['query', 'database']}
    ...
    def test_inference_twoway(self, idx, args, local_img_model, local_txt_model):
        ...
        re_BI, re_BT, re_L, qu_BI, qu_BT, qu_L = compress(
            self.databaseloader, self.testloader, model_img, model_txt, device)
        MAP_I2T = calculate_top_map(
            qu_B=qu_BI, re_B=re_BT, qu_L=qu_L, re_L=re_L, topk=50)
        MAP_T2I = calculate_top_map(
            qu_B=qu_BT, re_B=re_BI, qu_L=qu_L, re_L=re_L, topk=50)
...
if __name__ == '__main__':
    ...
    dataset_train, dataset_test, client_idcs_train, client_idcs_retrieval, client_idcs_qery = prepare_data_noniid(
        args=args)
    ...
    for _ in range(args.num_users):
        private_model = Blip24Fed()
        private_model_list.append(private_model)

    global_model = Blip24Fed()
    ...
    for round in range(args.rounds):
        ...
        for idx in idxs_users:
            print("client", idx)
            local_model = LocalUpdateDJSRH(
                args=args, id=idx, dataset=dataset_train, idxs=client_idcs_train[idx])
            ...
            w_img, w_txt, private_w_img, private_w_txt, local_centroid = local_model.update_weights_UCCH_new(args, idx, global_avg_protos_for_this_client, model_img=copy.deepcopy(
                global_model_img), model_txt=copy.deepcopy(global_model_txt), private_img_model=copy.deepcopy(private_img_model_list[idx]), private_txt_model=copy.deepcopy(private_txt_model_list[idx]))
        ...
        for idx in idxs_users:
            private_img_model_list[idx].load_state_dict(
                private_weights_img[idx])
            private_txt_model_list[idx].load_state_dict(
                private_weights_txt[idx])

        global_model_img = copy.deepcopy(global_model_img)
        global_model_txt = copy.deepcopy(global_model_txt)

        global_model_img.load_state_dict(
            local_weights_list_img[0], strict=True)
        global_model_txt.load_state_dict(
            local_weights_list_txt[0], strict=True)

        if round % 2 == 0:
            with torch.no_grad():
                MAP_I2T_all, MAP_T2I_all = 0.0, 0.0
                for idx in range(args.num_users):
                    local_test = LocalTest(args=args, id=idx, dataloaders=dataset_test,
                                           idcs_retrieval=client_idcs_retrieval, idcs_query=client_idcs_qery)
                    local_img_model = copy.deepcopy(global_model_img)
                    local_txt_model = copy.deepcopy(global_model_txt)

                    MAP_I2T, MAP_T2I = local_test.test_inference_twoway(
                        idx, args, local_img_model, local_txt_model)
                    MAP_I2T_all += MAP_I2T
                    MAP_T2I_all += MAP_T2I
                ...
                # MAPi2t/MAPt2i/ ,MAP_I2T_a_ave,MAP_T2I_a_ave
                print('Test/MAPi2t@50/MAPt2i@50/user' +
                      str(MAP_I2T_ave), MAP_T2I_ave, round)
                if MAP_T2I_ave + MAP_I2T_ave > best:
                    best = MAP_T2I_ave + MAP_I2T_ave
                    file_name = '%s_%d_bit_latest_our.pth' % (
                        str(args.dataset), args.bit)
